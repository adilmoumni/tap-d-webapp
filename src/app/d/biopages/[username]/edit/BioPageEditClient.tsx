...
export default function BioPageEditClient() {
  ...
  const targetBioRef = useMemo(() => {
    const fromParams = readUsernameParam(params?.username).trim();
    const segments = (pathname ?? "").split("/").filter(Boolean);
    const fromPath =
      segments.length >= 4 &&
      (segments[0] === "d" || segments[0] === "dashboard") &&
      segments[1] === "biopages" &&
      segments[3] === "edit"
        ? decodeURIComponent(segments[2] ?? "")
        : "";

    // Static export can hydrate dynamic params as "placeholder".
    if (fromPath && fromPath !== "placeholder") return fromPath;
    return fromParams;
  }, [params, pathname]);
